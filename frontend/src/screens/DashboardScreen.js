                    ...
                    {received  && data.map(item => (
                        <tr key={item._id.$oid.toString()} class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <td class="px-6 py-4 text-right">
                                <button type='button' onClick={() => navigate('/report', { state: { prediction: item.result.toString(), name:item.name, id:item._id.$oid.toString(), timestamps:item.timestamps['$date'], userId:item.userId.$oid.toString() } })} class="font-medium text-blue-600 dark:text-blue-500 hover:underline">View</button>
                            </td>
                        </tr>
                    ))
